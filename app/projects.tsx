...
export default function ProjectsScreen() {
  ...
  const update = (id: string, patch: Partial<Project>) => setData({ ...data, projects: data.projects.map(p => p.id === id ? { ...p, ...patch } : p) });
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <Text style={styles.title}>Life Projects</Text>
      <Button title="Add project" onPress={() => setData({ ...data, projects: [...data.projects, newProject()] })} />
      {data.projects.map(p => (
        <Card key={p.id}>
          ...
          <Button title="Delete" onPress={() => setData({ ...data, projects: data.projects.filter(x => x.id !== p.id) })} />
        </Card>
      ))}
    </ScrollView>
  );
}
const styles = StyleSheet.create({ container: { flex: 1, backgroundColor: '#f2f2f7' }, content: { padding: 18, paddingTop: 64 }, title: { fontSize: 32, fontWeight: '800', marginBottom: 16 }, two: { flexDirection: 'row', gap: 10 }, flex: { flex: 1 } });
