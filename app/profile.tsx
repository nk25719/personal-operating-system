  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <Card variant="highlight">
        ...
        <Field label="Preferred name" value={data.preferences.preferredName ?? active.name} onChangeText={v => setData({ ...data, preferences: { ...data.preferences, preferredName: v } })} />
        <Field label="Tone" value={data.preferences.tone} onChangeText={v => setData({ ...data, preferences: { ...data.preferences, tone: v as any } })} placeholder="warm / direct / reflective / coach / minimal" />
        <Field label="Rotating personal prompts" value={data.preferences.rotatingPrompts.join('\n')} onChangeText={v => setData({ ...data, preferences: { ...data.preferences, rotatingPrompts: v.split('\n').map(x => x.trim()).filter(Boolean) } })} multiline />
      ...
      <Card>
        ...
        <Field label="Date of birth" value={data.lifeProfile.birthDate ?? ''} onChangeText={v => setData({ ...data, lifeProfile: { ...data.lifeProfile, birthDate: v } })} placeholder="YYYY-MM-DD" />
...
const styles = StyleSheet.create({
  ...
  note: { color: '#7c6f64', lineHeight: 21, marginTop: 8 }
});
